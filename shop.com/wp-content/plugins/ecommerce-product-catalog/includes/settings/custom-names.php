...
function custom_names_menu() {
	if ( current_user_can( 'manage_product_settings' ) ) {
        ...
        <a id="names-settings" class="nav-tab"
           href="<?php echo admin_url( 'edit.php?post_type=al_product&page=product-settings.php&tab=names-settings&submenu=single-names' ) ?>	"><?php _e( 'Front-end Labels', 'ecommerce-product-catalog' ); ?></a>
		<?php
	}
}
...
function custom_names_content() {
    ...
    <h3>
        <a id="single-names" class="element current"
           href="<?php echo admin_url( 'edit.php?post_type=al_product&page=product-settings.php&tab=names-settings&submenu=single-names' ) ?>"><?php _e( 'Single Item Page', 'ecommerce-product-catalog' ); ?></a>
        <a id="archive-names" class="element"
           href="<?php echo admin_url( 'edit.php?post_type=al_product&page=product-settings.php&tab=names-settings&submenu=archive-names' ) ?>"><?php _e( 'Listing Pages', 'ecommerce-product-catalog' ); ?></a>
        ...
        <div id="single_names" class="setting-content submenu">
            ...
            <form method="post" action="options.php">
                ...
                <table class="wp-list-table widefat product-settings-table" style="clear:right; text-align: left;">
                    <thead>
                    <th><strong><?php _e( 'Front-end Element', 'ecommerce-product-catalog' ); ?></strong></th>
                    <th><strong><?php _e( 'Front-end Text', 'ecommerce-product-catalog' ); ?></strong></th>
                    </thead>
                    <tbody>
					<?php
					do_action( 'single_names_table_start', $single_names );
					implecode_settings_text( __( 'Description Label', 'ecommerce-product-catalog' ), 'single_names[product_description]', $single_names['product_description'] );
					implecode_settings_text( __( 'Features Label', 'ecommerce-product-catalog' ), 'single_names[product_features]', $single_names['product_features'] );
					implecode_settings_text( __( 'Another Categories Label', 'ecommerce-product-catalog' ), 'single_names[other_categories]', $single_names['other_categories'] );
					implecode_settings_text( __( 'Return to Products Label', 'ecommerce-product-catalog' ), 'single_names[return_to_archive]', $single_names['return_to_archive'] );
        ...
        <div id="archive_names" class="setting-content submenu">
            ...
            <form method="post" action="options.php"><?php
                ...
                <table class="wp-list-table widefat product-settings-table"
                    ...
                    <thead>
                    <th><strong><?php _e( 'Front-end Element', 'ecommerce-product-catalog' ); ?></strong></th>
                    <th style="width:69%"><strong><?php _e( 'Front-end Text', 'ecommerce-product-catalog' ); ?></strong>
                    </th>
                    </thead>
                    <tbody>
					<?php
					implecode_settings_text( __( 'Main Listing Title Label', 'ecommerce-product-catalog' ), 'archive_names[all_products]', $archive_names['all_products'], null, 1, 'wide', null, $disabled );
					implecode_settings_text( __( 'Categories Header Label', 'ecommerce-product-catalog' ), 'archive_names[all_main_categories]', $archive_names['all_main_categories'], null, 1, 'wide', null, $disabled );
					implecode_settings_text( __( 'Subcategories Header Label', 'ecommerce-product-catalog' ), 'archive_names[all_subcategories]', $archive_names['all_subcategories'], null, 1, 'wide', null, $disabled );
					implecode_settings_text( __( 'Category Prefix Label', 'ecommerce-product-catalog' ), 'archive_names[all_prefix]', $archive_names['all_prefix'], null, 1, 'wide', null, $disabled );
					implecode_settings_text( __( 'Category Products Header Label', 'ecommerce-product-catalog' ), 'archive_names[category_products]', $archive_names['category_products'], null, 1, 'wide', null, $disabled );
					implecode_settings_text( __( 'Next Page Label', 'ecommerce-product-catalog' ), 'archive_names[next_products]', $archive_names['next_products'], null, 1, 'wide', null, $disabled );
					implecode_settings_text( __( 'Previous Page Label', 'ecommerce-product-catalog' ), 'archive_names[previous_products]', $archive_names['previous_products'], null, 1, 'wide', null, $disabled );
