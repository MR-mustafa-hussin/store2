...
function is_ic_product_search( $query = null ) {
	...
	if ( ( ( empty( $query ) && is_search() ) || ( ! empty( $query ) && $query->is_search() ) ) && isset( $_GET['post_type'] ) && is_ic_valid_post_type( $_GET['post_type'] ) ) {
		return true;
	}
	...
	if ( isset( $session['filters'] ) && ! empty( $session['filters'] ) ) {
		...
		if ( ! empty( $exclude ) ) {
			foreach ( $session['filters'] as $filter_name => $filter_value ) {
				if ( ! in_array( $filter_value, $exclude ) && ( empty( $_GET[ $filter_name ] ) || ! in_array( $_GET[ $filter_name ], $exclude ) ) ) {
					return true;
				}
			}
