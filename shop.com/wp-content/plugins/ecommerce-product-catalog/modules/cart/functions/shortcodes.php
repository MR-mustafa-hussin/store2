	...
	static function shopping_cart() {
		...
		if ( $shopping_cart_settings['cart_page_template'] == 'no_qty' ) {
			...
			if ( ! empty( $shopping_cart_settings['contnue_shopping_label'] ) ) {
				$form .= '<a class="continue_shopping ' . $button_class . '" href="' . $back_url . '">' . $shopping_cart_settings['contnue_shopping_label'] . '</a>';
			}
			$form .= '</div>';
		}
		...
		$form .= '<input class="to_cart_submit ' . $button_class . '" type="submit" value="' . esc_attr( $shopping_cart_settings['place_order_label'] ) . '"></div>';
