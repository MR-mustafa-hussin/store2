...
function ic_responsive_filters_bar(open) {
    ...
    if (open == "0" || jQuery(".product-list").hasClass("responsive") || jQuery(".product-list").hasClass("grouped-filters")) {
        ...
        if (!responsive_filters && jQuery("#product_filters_bar .filter-widget").length > 0) {
            if (!responsive_filters && jQuery("#product_filters_bar .filter-widget:not(.widget_product_categories):not(.product_search)").length > 0) {
                var responsive_filters = '<div class="responsive-filters" style="display: none">';
